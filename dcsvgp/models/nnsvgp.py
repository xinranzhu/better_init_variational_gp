...
from .variational_strategy_nn_mean_predictor import VariationalStrategyNNMeanPredictor
...
def train_gp(model, train_x, train_y, 
    ...
    for i in range(num_epochs-previous_epoch):
        ...
        if i % 50 == 0:
            print(f"loss: {loss.item():.3f}, lengthscale: {model.covar_module.lengthscale.mean().item():.3f}")
