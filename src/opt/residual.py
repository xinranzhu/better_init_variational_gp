...
def lstsq_jacobian(u, x, y, theta, phi, Drho_phi, Dtheta_phi, cache):
    m, d = u.shape
    n, d = x.shape
    ...
    JA_ex = torch.cat([JA, torch.zeros(m, m * d + m, device=JA.device)], dim=0)
    z = torch.matmul(JA_ex.T, r)
    JAtr = torch.zeros(m, m * d + 1, device=JA.device)
