...
class VariationalStrategyDecoupledFeatureExtractors(_VariationalStrategyDecoupledFeatureExtractors):
    ...
    def __init__(self, model, inducing_points_mean, inducing_points_covar, variational_distribution, learn_inducing_locations=True):
        super().__init__(model, inducing_points_mean, inducing_points_covar, variational_distribution, learn_inducing_locations)
    ...
    def prior_distribution(self):
        ...
        # tend = time.time()
        # print(f"total time in prior dist: {(tend-t0):.2e}")
        return res

    def forward(self, x, inducing_points_mean, inducing_points_covar, inducing_values, variational_inducing_covar=None, **kwargs):
        x_mean = kwargs['x_mean']

        # Compute full prior distribution
        full_inputs = torch.cat([inducing_points_covar, x], dim=-2)
        full_output = self.model.forward(full_inputs)
        full_covar = full_output.lazy_covariance_matrix


        # Covariance terms
        num_induc = inducing_points_covar.size(-2)
        ...
        full_inputs = torch.cat([inducing_points_mean, x_mean], dim=-2)
        full_output = self.model.forward(full_inputs)
        full_covar = full_output.lazy_covariance_matrix

        # Covariance terms
        num_induc = inducing_points_mean.size(-2)
        induc_induc_covar_mean = full_covar[..., :num_induc, :num_induc].add_jitter()
        induc_data_covar_mean = full_covar[..., :num_induc, num_induc:].evaluate()
